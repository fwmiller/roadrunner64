...
load_parttab(uint8_t *mbr, part_t parttab) {
    ...
    for (i = 0, p = parttab, partent = PART_ENT_1; i < PARTS;
         i++, p++, partent += PART_ENT_SIZE) {
...
dump_part(int partno, part_t p) {
    printf("%c", (p->active ? '*' : ' '));
    printf("%3d  %5u  %4u %6u %5u  %4u %6u   %6u %11u  ", partno, p->st,
           p->sh, p->ss, p->et, p->eh, p->es, p->off, p->size);
    switch (p->sys) {
    case PART_SYS_NONE:
        printf("unused");
        break;
